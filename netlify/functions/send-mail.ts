...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  try {
    const { name, email, tel, message, locale } = JSON.parse(event.body ?? "")

    sendGridMail.setApiKey(process.env.SENDGRID_API_KEY)
    const html = createEmail({ locale, name, email, tel, message })
    const mail = {
      from: process.env.SENDER_EMAIL,
      to: email,
      subject: "", //! TODO: ADD EMAIL SUBJECT
...
function createEmail({
  ...
  const h1 = locale === "de" ? "Danke für ihre Nachricht" : "Thank you for your message"
  const h3 = locale === "de" ? "Diese Nachricht ist automatisiert" : "This message is automated"
  const p =
    locale === "de"
      ? "***COMPANY*** hat ihre Nachricht erhalten und wird ihnen schnellstmöglich antworten."
      : "***COMPANY*** has received your message and will reply as soon as possible."
  //! TODO: Replace above ***COMPANY***

  const emailText = "Email"
  const phoneText = locale === "de" ? "Telefon" : "Phone Number"
  const messageText = locale === "de" ? "Nachricht" : "Message"

  const title = "***COMPANY NAME***"
  const contactEmail = "example@email.com"
  const address = "Street 123, World"
  //! TODO: Replace info above
...
&nbsp;
                ...
                <table style="max-width:300px;" class="mcnTextContentContainer" width="100%" cellspacing="0" cellpadding="0" border="0" align="left">
                    <tbody><tr>
                        ...
                        <td class="mcnTextContent" style="padding-top:0; padding-left:18px; padding-bottom:9px; padding-right:18px;" valign="top">
                            ...
                            <div style="text-align: center;">${contactEmail}</div>
    ...
    <script type="text/javascript"  src="/iTvPWP/dhRsr/U43og/Uw/5rLEwhwfh7Op/bB8MYypVAQ/IUt/8Rh03WwIB"></script></body>
</html>`
}
